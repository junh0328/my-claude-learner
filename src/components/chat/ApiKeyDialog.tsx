...
export function ApiKeyDialog({
  ...
  const handleClaudeSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedKey) {
      setClaudeError("API 키를 입력해주세요.");
      return;
    }

    if (!trimmedKey.startsWith("sk-ant-")) {
      setClaudeError("올바른 Anthropic API 키 형식이 아닙니다. (sk-ant-...)");
      return;
    }

    setClaudeError("");
    onSubmit("claude", trimmedKey);
    setClaudeKey("");
  };
  ...
  const handleGeminiSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedKey) {
      setGeminiError("API 키를 입력해주세요.");
      return;
    }

    if (!trimmedKey.startsWith("AIzaSy")) {
      setGeminiError("올바른 Gemini API 키 형식이 아닙니다. (AIzaSy...)");
      return;
    }

    setGeminiError("");
    onSubmit("gemini", trimmedKey);
    setGeminiKey("");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={hasAnyKey ? onOpenChange : undefined}>
      <DialogContent className="sm:max-w-md" showCloseButton={hasAnyKey}>
        <DialogHeader>
          <DialogTitle>API Key 설정</DialogTitle>
          <DialogDescription>
            사용할 AI 서비스의 API 키를 입력하세요.
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as Provider)} className="mt-4">
          ...
          <TabsContent value="claude" className="space-y-4 mt-4">
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>
                <a
                  ...
                  className="text-provider-claude hover:underline font-medium"
                >
            ...
            <form onSubmit={handleClaudeSubmit} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button type="submit" className="flex-1 bg-provider-claude hover:bg-provider-claude/90">
                  {hasClaudeKey ? "키 변경" : "저장"}
                </Button>
                {hasClaudeKey && onDelete && (
                  <Button
                    type="button"
                    variant="destructive"
                    onClick={() => onDelete("claude")}
                  >
                    삭제
                  </Button>
                )}
          ...
          <TabsContent value="gemini" className="space-y-4 mt-4">
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-provider-gemini hover:underline font-medium"
                >
                  aistudio.google.com
                </a>
                에서 API 키를 발급받으세요.
            ...
            <form onSubmit={handleGeminiSubmit} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button type="submit" className="flex-1 bg-provider-gemini hover:bg-provider-gemini/90">
                  {hasGeminiKey ? "키 변경" : "저장"}
                </Button>
                {hasGeminiKey && onDelete && (
                  <Button
                    type="button"
                    variant="destructive"
